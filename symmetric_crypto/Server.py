...
def dealWithClient(conn, addr, key):
    print(datetime.now(), " Client recived from IP: ",
          addr[0], " Port: ", addr[1])

    # recieve the initial message from the client including cipher and IV
    initialMessage = conn.recv(256).decode("utf-8")
    # Split the initial message into cipher and IV
    cipher, iv = initialMessage.split(" ", 1)
    ...
    if (cipher == "aes256"):
        ...
        cipher = Cipher(algorithms.AES(key.encode("utf-8")),
                        modes.CBC(ivbytes), backend=backend)
    elif (cipher == "aes128"):
        backend = default_backend()
        cipher = Cipher(algorithms.AES(key[0:16].encode(
            "utf-8")), modes.CBC(ivbytes), backend=backend)  # Only want first half of the key
    elif (cipher != "none"):
    ...
    if (cipher != None):
        ...
        conn.send(EasyCrypto.encryptAndPad(
            cipher, key.encode("utf-8")))  # send key to client
        try:
            keycheck = EasyCrypto.decryptAndUnpad(
                cipher, conn.recv(256)).decode("utf-8")  # recieve client key
        except ValueError:
    ...
    if command == "read":
        if os.path.isfile(fileName):
            ...
            print(
                "\tError: The file requested by the client doesn't exist or isn't a file")
            conn.send(EasyCrypto.encryptAndPad(
                cipher, "The file requested by the client doesn't exist or isn't a file".encode("utf-8")))
            print("\t", datetime.now(), " done")
            conn.close()
            return False
    else:
        # changed the access to work on both Windows and non-Windows machines, will error if it cannot write to that location
        # Is the location writeable. Changed this to this path because with the filename if it didn't exist it would always be false
        if os.access("../", os.W_OK) and platform.system() != "Windows" or os.access("/", os.W_OK) and platform.system() == "Windows":
            conn.send(EasyCrypto.encryptAndPad(cipher, "Good".encode("utf-8")))
            print("\t", datetime.now(), " Beginning to recieve file: ", fileName)
            EasyCrypto.recieveFile(conn, cipher, fileName)
            print("\t", datetime.now(),
                  " File tranfer is finished, connection closing")
            print("\t", datetime.now(), " done")
            conn.close()
        else:
            ...
            conn.send(EasyCrypto.encryptAndPad(
                cipher, "The server does not have permission to write to that location".encode("utf-8")))
            print("\t", datetime.now(), " done")
            conn.close()
            return False
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Server for CPSC526 Assignment 3")
    parser.add_argument(
        "port", type=int, help="The target port for the client to connect to")
    parser.add_argument(
    ...
    # Need to make key 256bits (32 bytes) in case AES256 is used
    if args.key == None:
        key = ''.join(choice(ascii_lowercase+ascii_uppercase+digits)
                      for i in range(32))
